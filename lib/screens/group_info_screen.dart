...
class _GroupInfoScreenState extends State<GroupInfoScreen> {
  ...
  Future<void> _addMembers() async {
    ...
    final candidates = friends.where((f) => !_group.members.contains(f.id)).toList();
    if (!mounted) return;
            ...
            child: Column(
              ...
              children: [
                ...
                GradientButton(
                  ...
                  child: Text('Ekle (${selected.length})', style: AppText.button),
                ),
              ],
            ),
          ),
    ...
    if (result == true && selected.isNotEmpty) {
      MessagingService().addGroupMembers(groupId: _group.id, memberIds: selected.toList());
    }
  }
  ...
  Future<void> _confirmRemove(String memberId, String displayName) async {
    ...
    if (confirmed == true) {
      MessagingService().removeGroupMember(groupId: _group.id, memberId: memberId);
    }
  }
            ...
            child: ListView(
              ...
              children: [
                Center(
                  child: Column(
                    children: [
                      ...
                      Text('${_group.members.length} üye', style: TextStyle(color: AppColors.textMuted)),
                    ],
                  ),
                ),
                ...
                Row(
                  ...
                  children: [
                    Text('Üyeler', style: AppText.subheading.copyWith(fontSize: 15)),
                    if (_isAdmin)
                      TextButton.icon(
                              ...
                              onSelected: (value) {
                                if (value == 'toggle-admin') {
                                  MessagingService().setGroupAdmin(
                                      groupId: _group.id, memberId: memberId, isAdmin: !isAdminRow);
