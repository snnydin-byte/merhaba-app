...
void main() {
  ...
  group('lastSeenLabel', () {
    ...
    test('çevrimiçiyse son görülme yerine "Çevrimiçi" döner', () {
      expect(
        lastSeenLabel(online: true, lastSeen: now.subtract(const Duration(days: 5)), now: now),
        'Çevrimiçi',
      );
    });

    test('hiç son görülme kaydı yoksa "Çevrimdışı" döner', () {
      expect(lastSeenLabel(online: false, lastSeen: null, now: now), 'Çevrimdışı');
    });

    test('1 dakikadan az önce ise "Az önce çevrimiçiydi" döner', () {
      expect(
        lastSeenLabel(online: false, lastSeen: now.subtract(const Duration(seconds: 30)), now: now),
    ...
    test('dakikalar içinde doğru sayıyı gösterir', () {
      expect(
        lastSeenLabel(online: false, lastSeen: now.subtract(const Duration(minutes: 15)), now: now),
    ...
    test('saatler içinde doğru sayıyı gösterir', () {
      expect(
        lastSeenLabel(online: false, lastSeen: now.subtract(const Duration(hours: 3)), now: now),
    ...
    test('günler içinde doğru sayıyı gösterir', () {
      expect(
        lastSeenLabel(online: false, lastSeen: now.subtract(const Duration(days: 2)), now: now),
    ...
    test('bir haftadan uzun süredir çevrimdışıysa genel bir mesaj döner', () {
      expect(
        lastSeenLabel(online: false, lastSeen: now.subtract(const Duration(days: 30)), now: now),
